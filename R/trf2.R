...
#' Baixar e parsear processos do TRF2
#'
#' @description Diferentemente da maioria dos downloaders/parsers,
#' a organizacao do TRF2 faz com que esses passos precisem estar juntos.
...
#' @examples
#' \dontrun{
#' download_and_parse_trf2("0011542-27.2017.4.02.0000")
#' }
#'
#' @export
download_and_parse_trf2 <- function(id, path_rds = ".") {

  connect <- function(id) {
    u0 <- "http://portal.trf2.jus.br/portal/consulta/cons_procs.asp"
  ...
  download_and_parse_jfrj_ <- function(id) {
    ...
    u_lawsuit <- list(
      ...
      purrr::map(~sprintf("%s%s?CodDoc=%s&IDNumConsProc=%s",
        "http://portal.trf2.jus.br/portal/consulta/",
